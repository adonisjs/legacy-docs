...
NOTE: There is no need to define relationship on both the models. Setting it on a single model works as expected. Also, lucid relationships are not bound to SQL schema.
...
2. Also a *car* can have many *owners* during its lifespan.
...
NOTE: After defining `pivotModel`, you cannot call `pivotTable` and `withTimestamps` methods. Instead, you are required to set these values on the pivot model.
...
You can also add runtime constraints by calling query builder methods as you would do with a typical query.
...
In order to pass query constraints, you must give an object.
...
==== attach
The `attach` method works with the belongsToMany relationship to attach a relationship to the `pivot` table.
...
==== sync
The `sync` method is a shortcut for calling `detach` and then `attach` methods. It can be used for updating a pivot table.
...
In the case of updating the pivot table, you can fetch the `query` instance for pivot table only.
