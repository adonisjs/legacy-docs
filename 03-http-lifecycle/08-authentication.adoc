...
5. Authentication provider will make use of the link:encryption-and-hashing#_hashing_values[Hash] module to verify passwords. Make sure to link:database-hooks#_defining_hooks[hash your passwords] before saving them to the database.


== Setup
Authentication provider comes pre-installed with `full-stack` and `api-only` boilerplates. If not installed, follow the defined steps.
...
=== Login user
Let's start by creating the `login` method on the UserController, which will take the user credentials and log in the user if they are correct.
...
Yes, that's all you really need to do in order to login a user using their `email` and `password`.

=== Showing profile
Let's write the `show` method too, which will return the user profile back as JSON.
...
The `auth.user` is the instance of the *User model*, so we can just return it back. Also, we want to make sure the `id` received as params is same as currently logged in user id.

== Sessions
The *session authenticator* exposes following methods to login or authenticate a user.

==== attempt(uid, password)
Attempt to log in a user using their `uid` and `password`. It will throw an exception if unable to find the user or if password mismatch.
...
==== login(user)
Login a user using the `user` model instance. This method doesn't verify anything and simply mark the user as logged in.
...
==== loginViaId(id)
Login a user using their id. This method will query the database to make sure the user exists.
...
==== remember
Anytime you can call methods like `attempt`, `login` or `loginViaId`, you can chain the `remember` method to make sure that users are logged in again even after closing their browser.

NOTE: The `remember` method will create a token for the user inside the `tokens` table. If you ever want to revoke the long-lived session of a particular user, simply set `is_revoked` to true.
...
==== getUser
This method will call the `check` method internally and returns the user details if they are logged in.
...
== Basic Auth
The basic authentication is a stateless auth, where the end-user is supposed to pass the credentials on each request, so there is no concept of `login` and `logout`.

==== check
Check to see if the user has passed `basic auth` credentials in the request header or not. Also, this method will verify the user existence and their password

NOTE: Set the `Authorization = Bearer <credentails>`  header in order to authenticate the request.
...
try {
  ...
  response.send('Credentails missing')
}
----
...
== JWT
The link:https://jwt.io/[jwt authentication] is an industry standard to implement stateless authentication on the basis of tokens and AdonisJs has out of the box support for JWT.

NOTE: Set the `Authorization = Bearer <token>`  header in order to authenticate the request.
...
Generate JWT token for a given user. Optionally you can pass a custom object to be encoded within the token. Passing `jwtPayload=true` will encode the user object within the token.
...
==== newRefreshToken
When generating a new `jwt` token, auth provider will not re-issue a new refresh token and instead uses the old one. If you want, you can also regenerate a new refresh token.
...
[source, js]
----
await auth.tokensList()
----

== Personal API tokens
The personal API tokens become popular by link:https://github.com/blog/1509-personal-api-tokens[Github], where a user can generate a token to be used in scripts, where it's not possible to manually type the *email* and *password*.
...
The auth provider makes it so simple to use multiple *authenticators* at once. You can switch to a different authenticator at runtime by calling the `authenticator` method.
...
== Revoking tokens
Almost every scheme stores some sort of tokens in the database. These tokens are mainly used for extended authentication. For example:

1. *Refresh tokens* when the actual token has been expired.
2. *Remember me token* when user actual session has expired, this token is used to re-login them transparently.
3. *API tokens* which are sort of like passwords.

If you ever have a security breach, feel free to set `is_revoked = true`, to revoke all the tokens for a given or for all the users.
...
This is done to ensure, that if someone gets access to your database, they will not be able to use these tokens directly and have to figure out how to encrypt them using the secret key.
