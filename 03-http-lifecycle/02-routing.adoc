...
== Route methods/verbs
Restful routes makes use of different HTTP methods to indicate the type of request. For example: `POST` method is used to create a record and `GET` is used to fetch record(s).
...
== Routing for SPA's
Quite often you find yourself creating an SPA ( single page application ), where you want to render a single view from the server and handle routing on front-end using your favorite frontend framework.
...
== Route formats
Route formats opens up a new way for link:https://en.wikipedia.org/wiki/Content_negotiation[Content negotiation, window="_blank"], where you can accept the response format as part of the URL.
...
Passing `true` as the second parameter will make sure that the client defines one of the expected formats, otherwise, a 404 is thrown.

== Route resources
If you like building web apps around REST conventions then *route resources* will help you in defining conventional routes by writing less code.

NOTE: It is required to bind a *Controller* to the resource. Binding a closure will throw exception.
...
The `Route.resource` method under the hood, will create a total of 7 routes
...
==== only([names])
Remove all other routes but not the one's passed to the `only` method.
...
Since attaching auth middleware to all the routes is not desired, you can customize the behavior by passing a *Closure* which should return an array of middleware to applied to each route within the resource.
...
Route
  .resource('users', 'UsersController')
  .middleware((route) => {
    if (['users.store', 'users.update', 'users.delete'].indexOf(route.name) > -1) {
      return ['auth']
    }
    return []
  })
...
=== Resource Formats
Also you can define the formats for all the resourceful routes, just like the way you will do for a single route or a group of routes.
...
TIP: You can define the domain on a single route as well, but generally it's a good idea to group domain specific routes together.
