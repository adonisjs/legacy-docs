...
Also, you can use `response.json` which is same as `response.send` but more explicit that the response is in JSON format.
...
==== safeHeader
Only set the header if it doesn't exist.
...
Redirect to a route. This method will build the URL automatically from the route name or controller method.
