...
Hooks are the actions you perform before or after a specified database operation. Hooks plays a major role in keeping your codebase DRY and easy to reason about.
...
Now everytime we create a user, the `beforeCreate` hook closure will be executed with the current instance of the user and will hash the password.

=== Hook file
It's always nice to keep extracted pieces of code into their own files. AdonisJs comes with a dedicated directory `app/Models/Hooks` to store model hooks.
...
Which means we can now remove the inline code from the model and instead reference this file and method.
...
| afterDelete | After a row has been removed.
| afterFind | After a single row is fetched from the database.
| afterFetch | After the `fetch` method is executed. The hook method will receive an array of model instance.
|====
