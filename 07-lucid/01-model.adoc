...
The knex query builder underlying Lucid defines methods `count()`, `avg()`, `sum()`, `min()`, and `max()`. To avoid confusion and naming collisions, Lucid prefixes its aggregate helper methods with `get`.
