...
== Aggregate helpers
Lucid aggregate helpers provide shortcut access to common aggregate queries. These same queries can be written long-form using the query builder interface.  come with a bunch of static methods to do common operations without using the query builder interface. Short and long-form examples are provided for each helper method.

There is no need to call `fetch` when using one of the following static methods. These methods end the query builder chaining and return a value.
...
await User.query().rowsCount()

# Return count of selected records (long form)
(await User.query().count('* as total'))[0].total
----
...
await User.sum('friends_count')

# Return sum of selected records
await User.query().sum('friends_count')

# Return sum of selected records (long form)
(await User.query().sum('friends_count as total'))[0].total
----
