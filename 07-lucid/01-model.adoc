...
== Aggregate helpers
Lucid aggregate helpers provide shortcut access to common aggregate queries without using the query builder interface. These same queries can be written long-form using the query builder interface. Short and long-form examples are provided for each helper method.
...
The knex query builder underlying Lucid defines methods `count()`, `avg()`, `sum()`, `min()`, and `max()`. To avoid confusion and naming collisions, Lucid prefixes its aggregate helper methods with `rows`.

==== rowsCount(columnName = '*')
...
await User.rowsCount()

// Return count of selected records (active users, for example)
await User.query().where('is_active', '=', 1).rowsCount()
...
await User.rowsSum('friends_count')

// Return sum of selected records
await User.query().where('is_active', '=', 1).rowsSum('friends_count')
...
// Return sum of all records in table
await User.rowsMin('friends_count')

// Return sum of selected records
await User.query().where('is_active', '=', 1).rowsMin('friends_count')
...
// Return sum of all records in table
await User.rowsMax('friends_count')

// Return sum of selected records
await User.query().where('is_active', '=', 1).rowsMax('friends_count')
...
// Return sum of all records in table
await User.rowsAvg('friends_count')

// Return sum of selected records
await User.query().where('is_active', '=', 1).rowsAvg('friends_count')
