...
Relationships are the core of data driven apps. Whether is about maintaing *posts* written by a given *author* or *cars* from a given *company*.

Lucid expressive API makes the process of associating and fetching these relations so simple and intuitive even without touching the SQL schema.
...
NOTE: The `posts` parameter passed as the 2nd parameter is the reference to the `posts` method on the User model and is always required to relationship to work.
...
On the other hand eagerloading will make total of 2 database queries to fetch all the users and their related posts.
