...
<2> Then we call the previously defined `profile()` to fetch the related profile for the given user.
...
<1> Now when you fetch the related profile for a given user, it will only include the record where 'is_active=true'.
