...
When you have an environment variable that is required for running the application, you may use a `getOrFail` method which will throw an error, if the variable is not set.

NOTE: If you want your application to fail quickly at boot time, if environment variable is missing, then limit access to environment variables from inside the configuration files only and do not to use `Env` provider anywhere else in the app. This is a highly recommended approach.
