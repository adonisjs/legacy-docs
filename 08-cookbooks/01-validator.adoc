...
class UserController {
  ...
  async update ({ request, session, response, params }) {
    ...
    const rules = {
      email: `unique:users,email,id,${userId}` <1>
    }
...
== Extending Validator
Quite often you have the requirement of extending the *Validator Provider* by adding new validation rules. You can make use of the `extend` method provided by link:http://indicative.adonisjs.com/#indicative-extending[Indicative].

==== Application Specific
For application specific rules you can make use of `app/hooks.js` file to listen for the *providersBooted* event and register your custom rules.
...
const adult = async function(data, field, message, args, get) {
...
hooks.after.providersBooted(() => {
  ...
  Validator.extend('adult', adult, 'You must be an adult')
})
----
...
class ExtendsValidatorProvider extends ServiceProvider {
  async exists (data, field, message, args, get) {
    ...
    const result = await Database.table(table).where(column, value).first()
    if (!result) {
      throw message
    }
  }
  ...
  async boot () {
    // register bindings
    const Validator = use('Validator')
    Validator.extend('exists', this.exists, '{{field}} does not exist in database')
  }
}

module.exports = ExtendsValidatorProvider
----
Don't forget register your provider in `app/start.js` then you can make use of the above defined `exists` like any other validation rule.