...
In this guide, we will create a simple number guessing game as a way to learn more about the framework. By the end of this guide, you will know how to use *views*, *create new routes* and *bind controllers* to them.
...
== Game story
The number guessing takes an input from the user and matches it against a random number. If the number matches, it's called a match, otherwise it's a pass.

To keep it simple, we will accept the user guessed number as the query string in the URL.

== Setup
Let's create a new project using `adonis` command. We will create a slim project, since we do not need a database or models for this app.
...
Now `cd` into the created directory and make sure you are able to run the application using `adonis serve` command.
...
Now if we visit link:http://127.0.0.1:3333?number=5[127.0.0.1:3333?number=5] and keep on changing the `number` between 1-20, we will see *Matched* or the *Match failed* statement.
...
TIP: If you can any questions or concerns, please join us on link:https://forum.adonisjs.com/[discourse, window="_blank"] to be a part of our small and helpful community.
